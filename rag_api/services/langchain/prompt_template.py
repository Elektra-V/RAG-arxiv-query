...
def get_baseline_prompt_template() -> str:
    ...
    return (
        ...
        "## Query Validation (CRITICAL - CHECK FIRST)\n\n"
        "**BEFORE using any tools, you MUST validate the query.**\n\n"
        "**Invalid queries include:**\n"
        "- Gibberish text (e.g., 'asdfghjkl', 'qwertyuiop')\n"
        "- Random numbers (e.g., '123456', '789012')\n"
        "- Nonsensical combinations (e.g., 'xyzabc123', 'nonexistenttopic123', 'nonexistent123456')\n"
        "- Queries containing 'nonexistent' + numbers (e.g., 'nonexistenttopic123', 'nonexistent123456')\n"
        "- Queries that are clearly not academic/scientific questions\n"
        "- Random keyboard mashing or meaningless text\n\n"
        "**If the query contains ANY of these patterns, DO NOT use any tools.**\n"
        "Instead, immediately respond with:\n"
        "TOOL_LOG:\n"
        "- rag_query: NOT_USED\n"
        "- arxiv_search: NOT_USED\n"
        "- llm_only: true\n"
        "- query_valid: false\n"
        "\n"
        "ANSWER:\n"
        "This query is not valid. Please provide a valid academic or scientific question.\n\n"
        ...
        "1. **FIRST**: Check if query is invalid (see Query Validation above). If invalid, DO NOT use tools.\n"
        "2. **THEN**: If valid, briefly decide which tool to call first and why (based on the strategy above). Then call the tool(s) as needed.\n\n"
